import React, { Component } from 'react';
import { injectIntl } from 'react-intl';
import { Modal } from 'reactstrap';
import { connect } from 'react-redux';
import ActionForm from './ActionForm';
import { toggleAreaDomesticModal, changeTypeAreaDomesticModal, addAreaDomesticItem, updateAreaDomesticItem } from '../../../redux/actions';
import PropTypes from 'prop-types';
import { MODAL_ADD, MODAL_EDIT, MODAL_VIEW } from '../../../constants/defaultValues';

class Action extends Component {

  handleSubmit = values => {
    switch (this.props.modalType) {
      case MODAL_ADD:
        this.props.addAreaDomesticItem(values);
        break;
      case MODAL_EDIT:
        this.props.updateAreaDomesticItem(values);
        break;
      case MODAL_VIEW:
        this.props.changeTypeAreaDomesticModal(MODAL_EDIT);
        break;
      default:
        break;
    }
  }

  toggleModal = () => {
    this.props.toggleAreaDomesticModal();
  }

  render() {
    const { modalType } = this.props;
    let className = 'success';
    switch (modalType) {
      case MODAL_ADD:
        className = 'success';
        break;
      case MODAL_EDIT:
        className = 'primary';
        break;
      case MODAL_VIEW:
        className = 'info';
        break;
      default:
        break;
    }

    return (
      <Modal
        isOpen={this.props.modalOpen}
        toggle={this.toggleModal}
        className={`modal-dialog--${className} modal-dialog--header`}
      >
        <ActionForm onSubmit={this.handleSubmit} />
      </Modal>
    );
  }
}

Action.propTypes = {
  modalType: PropTypes.string,
  updateAreaDomesticItem: PropTypes.func.isRequired,
  toggleAreaDomesticModal: PropTypes.func.isRequired,
  addAreaDomesticItem: PropTypes.func.isRequired,
  changeTypeAreaDomesticModal: PropTypes.func.isRequired,
}

const mapStateToProps = ({ pricingDomestic }) => {
  const { area } = pricingDomestic;
  const { modalType } = area;
  return {
    modalType
  }
}

export default injectIntl(connect(mapStateToProps, {
  addAreaDomesticItem,
  updateAreaDomesticItem,
  toggleAreaDomesticModal,
  changeTypeAreaDomesticModal
})(Action));