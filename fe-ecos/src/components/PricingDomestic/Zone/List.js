...
class List extends Component {
  ...
  renderHeader = (selected) => {
    ...
    return (
      <Fragment>
        <Can user={this.props.authUser.user} permission="zone_domestic" action="add">
          <Button
            color="success"
            onClick={(e) => this.toggleModal(e, 'add', null)}
            className="master-data-btn"
            size="sm"
          >{messages['pri_dom.add-new-zone']}</Button>
        </Can>
        <Action modalOpen={modalOpen} />
        <Can user={this.props.authUser.user} permission="zone_domestic" action="delete">
          {selected.length > 0 &&
            <Button
              color="danger"
              ...
              className="master-data-btn"
              size="sm"
            >{messages['pri_dom.delete-zone']}</Button>
          }
        </Can>
      </Fragment>
    )
      ...
      columns: [
        ...
        },
        {
          ...
          Cell: ({ original }) => {
            return (
              <Fragment>
                <Can user={this.props.authUser.user} permission="zone_domestic" action="edit" own={original.created_at}>
                  <Button color="info" size="sm" onClick={(e) => this.toggleModal(e, 'edit', original)}><span className="lnr lnr-pencil" /></Button> &nbsp;
                </Can>
                <Can user={this.props.authUser.user} permission="zone_domestic" action="delete" own={original.created_at}>
