const validate = values => {
  ...
  if (values.vas && values.vas.length) {
    ...
    values.vas.forEach((vas, vasIndex) => {
      const vasErrors = {}
      if (!vas || !vas.name) {
        vasErrors.name = 'pri_int.validate-vas-name-required';
        vasArrayErrors[vasIndex] = vasErrors
      }
      if (!vas || !vas.formula) {
        vasErrors.formula = 'pri_int.validate-vas-formula-required';
        vasArrayErrors[vasIndex] = vasErrors
      }
      if (vas && (vas.type !== undefined || vas.type !== '') && vas.type === 0 && (vas.min === undefined || vas.min === '') ) {
        vasErrors.min = 'pri_int.validate-vas-min-required';
        vasArrayErrors[vasIndex] = vasErrors
      }
      if (vas && vas.type && vas.spec.length === 0) {
        vasErrors.spec = 'pri_int.validate-vas-spec-required';
        vasArrayErrors[vasIndex] = vasErrors
      }
    })
