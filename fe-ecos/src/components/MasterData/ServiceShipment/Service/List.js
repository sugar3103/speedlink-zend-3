...
import { getServiceList, toggleServiceModal, deleteServiceItem } from "../../../../redux/actions";
...
class List extends Component {
  ...
  toggleModal = (e, type, status) => {
    e.stopPropagation();
    this.props.toggleServiceModal(type, status);
  }
  ...
  renderHeader = (selected) => {
    const { messages } = this.props.intl;
    const { modalOpen } = this.props.service;
    return (
      <Fragment>
        <Can user={this.props.authUser.user} permission="service" action="add">
          <Button
            color="success"
        ...
        <Can user={this.props.authUser.user} permission="service" action="delete">
          {selected.length > 0 &&
            <Button
              color="danger"
              ...
              className="master-data-btn"
              size="sm"
            >{messages['service.delete']}</Button>
          }
        </Can>
      </Fragment>
    )
  ...
  render() {
    ...
    const { items, loading, total } = this.props.service;
      ...
      columns: [
        ...
        },
        {
          ...
          Cell: ({ original }) => {
            return (
              <Fragment>
                <Can user={this.props.authUser.user} permission="service" action="edit" own={original.created_at}>
                  <Button color="info" size="sm" onClick={(e) => this.toggleModal(e, 'edit', original)}><span className="lnr lnr-pencil" /></Button> &nbsp;
                </Can>
                <Can user={this.props.authUser.user} permission="service" action="delete" own={original.created_at}>
